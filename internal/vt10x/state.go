...
// maxHistoryLines caps how many scrolled-off primary-screen lines are kept
// for scrollback. Oldest lines are dropped once the cap is exceeded.
...
func newState(w io.Writer, historyLimit int) *State {
	if historyLimit <= 0 {
...
func (t *State) scrollUp(orig, n int) {
	...
	// Only a scroll of the whole primary screen (not a TUI app's custom
	// scroll region, and not the alternate screen used by full-screen
	// programs like vim/htop) represents lines a user would expect to
	// find in scrollback, mirroring real terminal emulator behavior.
...
// HistoryLen returns the number of scrolled-off primary-screen lines
// available above the live grid.
func (t *State) HistoryLen() int {
	return len(t.history)
}

// HistoryCell returns the glyph at column x of scrollback line n, where n=0
// is the oldest retained line and n=HistoryLen()-1 is immediately above row
// 0 of the live grid. Caller must hold Lock/Unlock, same as Cell.
func (t *State) HistoryCell(n, x int) Glyph {
